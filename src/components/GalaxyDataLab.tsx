...
const G = 6.674e-11;
const c = 3e8;
const H0_SI = 73 * 1e3 / (3.0857e22); // 73 km/s/Mpc → s⁻¹
const OMEGA_M = 0.3;
const OMEGA_L = 0.7;
...
export default function GalaxyDataLab() {
  ...
  const histogramData = useMemo(() => {
    ...
    return counts.map((count, i) => {
      ...
      return {
        alpha: +center.toFixed(4),
        count,
        classification: classify(center),
      };
    });
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      {!data.length && (
        <div className="text-center py-16 text-muted-foreground border border-dashed border-border rounded-xl">
          ...
          <p className="text-sm">Upload a CSV or load sample data to begin analysis</p>
            ...
            <div className="bg-card/30 border border-border rounded-xl p-4">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 10 }}>
                  ...
                  <XAxis
                    dataKey="z" type="number" name="z"
                    tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                    label={{ value: "Redshift z", position: "bottom", fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                  />
                  <YAxis
                    dataKey="alpha" type="number" name="α"
                    tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                    label={{ value: "α", angle: -90, position: "insideLeft", fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                  />
                  <Tooltip
                    ...
                    content={({ payload }) => {
                      ...
                      return (
                        <div className="bg-card border border-border rounded-lg p-2 text-xs space-y-0.5">
                          ...
                          <div>z = {d.z}</div>
                          <div className="capitalize" style={{ color: CLASS_COLORS[d.classification] }}>
                            {d.classification}
                          </div>
                        </div>
                      );
                  ...
                  <Scatter data={data} shape="circle">
                    {data.map((d, i) => (
                      <Cell key={i} fill={CLASS_COLORS[d.classification]} r={5} />
                    ))}
                  </Scatter>
                </ScatterChart>
              </ResponsiveContainer>
            ...
            <div className="bg-card/30 border border-border rounded-xl p-4">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 10 }}>
                  ...
                  />
                  <YAxis
                    dataKey="btfr_ratio" type="number" name="V/M^¼"
                    tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                    label={{ value: "V / M^¼", angle: -90, position: "insideLeft", fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                  />
                  <Tooltip
                    content={({ payload }) => {
                      if (!payload?.length) return null;
          ...
          <div className="bg-card/30 border border-border rounded-xl p-4">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={histogramData} margin={{ top: 10, right: 20, bottom: 20, left: 10 }}>
                ...
                <XAxis
                  dataKey="alpha" type="number"
                  tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                  label={{ value: "α", position: "bottom", fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                />
                <YAxis
                  tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                ...
                <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                  {histogramData.map((d, i) => (
                    <Cell key={i} fill={CLASS_COLORS[d.classification]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
                    ...
                    <TableRow key={i} className="border-border">
                      ...
                      <TableCell className="text-xs text-center">
                        <span
                          ...
                          style={{ backgroundColor: CLASS_COLORS[d.classification] }}
                        />
